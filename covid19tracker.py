...
def main():
    ...
    parser.add_argument(
        ...
        choices=["asc", "desc"],
        default="desc",
        help="sorts the result set in with ascending or descending order",
    )
    parser.add_argument(
        "-y",
        "--yesterday",
...
def change_case(globalkey):
    ...
    return (
        "".join(["_" + i.lower() if i.isupper() else i for i in globalkey])
        .lstrip("_")
        .title()
    )
...
def allcountries(url):
    ...
    for key, value in data.items():
        ...
        table.add_row(change_case(key), str("{:,}".format(value)))
...
def countries(url, sortby, orderby):
    ...
    for country in data.keys():
        table.add_row(
            f"{country}",
            str("[red]{:,}[/red]".format(result[country]["deaths"]))
            if sortby == "deaths"
            else str("{:,}".format(result[country]["deaths"])),

            str("[magenta]{:,}[/magenta]".format(result[country]["cases"]))
            if sortby == "cases"
            else str("{:,}".format(result[country]["cases"])),

            str("[green]{:,}[/green]".format(result[country]["recovered"]))
            if sortby == "recovered"
            else str("{:,}".format(result[country]["recovered"])),
            str("[blue]{:,}[/blue]".format(result[country]["active"]))
            if sortby == "active"
            else str("{:,}".format(result[country]["active"])),
            str("[cyan]{:,}[/cyan]".format(result[country]["critical"]))
            if sortby == "critical"
            else str("{:,}".format(result[country]["critical"])),
            str("[green]{:,}[/green]".format(result[country]["population"]))
            if sortby == "population"
            else str("{:,}".format(result[country]["population"])),
            str("[red]{:,}[/red]".format(result[country]["todayDeaths"]))
            if sortby == "todayDeaths"
            else str("{:,}".format(result[country]["todayDeaths"])),
            str("[magenta]{:,}[/magenta]".format(result[country]["todayCases"]))
            if sortby == "todayCases"
            else str("{:,}".format(result[country]["todayCases"])),
        )
