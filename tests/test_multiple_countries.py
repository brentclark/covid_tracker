...
isocountrylist = ('AF', 'AX', 'AL', 'DZ', 'AS', 'AD', 'AO', 'AI', 'AQ', 'AG',
...
@pytest.mark.parametrize('isocountry, expect', [
        (random.choice(isocountrylist), 'country'),
        (random.choice(isocountrylist), 'casesPerOneMillion')])
def test_multiple_countries(mocker, isocountry, expect):
    print(f"Logging for country: {isocountry} and I expect '{expect}'.")
    assert expect in connect(f"{base_url}/{isocountry}")
